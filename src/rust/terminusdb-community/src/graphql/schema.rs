...
use juniper::{
    DefaultScalarValue, FromInputValue, GraphQLType, GraphQLValue, InputValue, ScalarValue,
};
            ...
            .map(|(field_name, field_definition)| {
                if let Some(document_type) = field_definition.document_type() {
                    match field_definition.kind() {
                        FieldKind::Required => registry.field::<TerminusType>(
                            field_name,
                            &(document_type.to_owned(), frames.clone()),
                        ),
                        FieldKind::Optional => registry.field::<Option<TerminusType>>(
                            field_name,
                            &(document_type.to_owned(), frames.clone()),
                        ),
                        FieldKind::Array => registry.field::<Vec<Option<TerminusType>>>(
                            field_name,
                            &(document_type.to_owned(), frames.clone()),
                        ),
                        _ => registry.field::<Vec<TerminusType>>(
                            field_name,
                            &(document_type.to_owned(), frames.clone()),
                        ),
                    }
